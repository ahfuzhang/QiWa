...
public class Utils
{
    public static async Task<bool> FileExistsAndNotEmptyAsync(string path)
    {
        ...
        try
        {
            ...
            try
            {
                int readLen = await stream.ReadAsync(buf.AsMemory(0, 1)).ConfigureAwait(false);  // ConfigureAwait 后续的任务在哪个上下文里无所谓
                return readLen == 1;
            }
            finally
            {
    ...
    public static async Task<System.ValueTuple<Common.RentedBuffer, Common.Error>> ReadAllAndRentAync(string inputPath)
    {
        ...
        try
        {
            ...
            {
                int read = await stream.ReadAsync(rent.Data!, totalRead, length - totalRead).ConfigureAwait(false);
                if (read == 0)
                {
                    break;
                }
        ...
        catch (Exception ex)
        {
            ...
            rent.Dispose();
            return (default, Common.Error.WithLoc(code: 4, message: $"Exception:{ex.Message}"));
        }
    }
}
