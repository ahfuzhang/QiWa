...
{
    public static byte[] Format(IReadOnlyList<Metric> metrics, IReadOnlyDictionary<string, string> extraLabels)
    {
        int initialSize = Math.Max(1024, metrics.Count * 128);
        var writer = new ArrayBufferWriter<byte>(initialSize);
    ...
    private static void AppendLabel(ArrayBufferWriter<byte> writer, string key, string value, ref bool wroteAny)
    {
    ...
    private static void AppendLong(ArrayBufferWriter<byte> writer, long value)
    {
    ...
    private static void AppendDouble(ArrayBufferWriter<byte> writer, double value)
    {
