...
/// <summary>
/// Prompt intent: 用参数化测试覆盖 TaskLogger.Info 的全部重载和日志级别分支，避免当前文件覆盖率过低。
/// </summary>
public class TaskLogger_InfoTests : TestBase
{
    ...
    public void Info_AllOverloads_GenerateCorrectOutput(int fieldCount)
    {
        ...
        const string expectedFile = "/tmp/tasklogger-info-tests.cs";
    ...
    {
        var logger = Logger.Get();
        const string expectedFile = "/tmp/tasklogger-info-skip-tests.cs";
    ...
    /// </summary>
    private static void InvokeInfo(TaskLogger logger, int fieldCount, string file, string member, int line)
    {
