...
/// <summary>
/// Prompt intent: 用参数化测试覆盖 ThreadLocalLogger.Fatal 的全部重载，避免当前文件覆盖率过低。
/// </summary>
public class ThreadLocalLogger_FatalTests : TestBase
{
    /// <summary>
    /// 提供 1 到 20 个字段数量，用于逐个覆盖 Fatal 的全部重载。
    /// </summary>
    ...
    public void Fatal_AllOverloads_GenerateCorrectOutput(int fieldCount)
    {
        ...
        const string expectedFile = "/tmp/threadlocal-fatal-tests.cs";
    ...
    /// <summary>
    /// 根据字段数量显式调用对应的 Fatal 重载，确保当前提示词关注的所有重载都进入测试覆盖。
    /// </summary>
    private static void InvokeFatal(ThreadLocalLogger logger, int fieldCount, string file, string member, int line)
    {
