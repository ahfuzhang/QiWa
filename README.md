...
* 异常处理：
  ...
  - 程序不需要时时刻刻注意下层代码抛出异常
    - 异常处理一般只有两个位置：
      - 某个库抛出了异常，使用这个库的时候，立即捕获这个库已知的异常。把异常转换为状态对象，返回给上层。（而不是继续把异常丢给上层）
      - 每个 request 处理的入口处加上异常处理。发生异常时可以向调用者返回 5xx 错误。并记录日志。
        - 如果出现这种情况，说明异常是所有人意料之外的。
* 支持的组件方面:
...
* 过载保护
  ...
  - 使用 time wheel, 超过的请求直接拒绝，不产生 Task
* utf-8 string
  - 全部基于 utf-8 string 来处理。避免频繁的 utf-8 / utf-16 的转换
* copy from golang
  - 字符串处理尽量是基于 utf-8 的
  - 尽量使用自定义的 error 对象来代替 throw 异常
  - 使用 Span<T> 这样的胖指针，与 golang 的 slice 对齐.
    - byte[] 会在切片操作的时候复制，而 Span 不会。
* 性能
  - 核心是关注并发模型，基于 CLR 的特点来做顶层设计
